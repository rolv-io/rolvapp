---
title: "Rolv Desktop Documentation (v2.1.3)"
date: "2026-01-21"
output:
  html_document:
    toc: true
...
For windows installation, you should download a file named __rolvdesktop-2.1.3 Setup.exe__.Now, let’s walk through the steps to install _rolv_ desktop app:

1. Locate the __rolvdesktop-2.1.3 Setup.exe__ file on your computer.
...
#### Mac OS
For mac os, you should download the file called `rolvdesktop-darwin-x64-2.1.3.zip`.
Afterwards, follow the steps below:

1. Move the rolvdesktop-darwin-x64-2.1.3.zip file to the directory where you want it to be installed.
...
a file named rolvdesktop_2.1.3_amd64.deb.  Now, let’s walk through the steps to install _rolv_ desktop app:

1. Open the terminal ( press Ctrl + Alt + T or search for ‘Terminal’ in your applications menu)

2. Move to the directory in which you downloaded rolvdesktop_2.1.3_amd64.deb. This can be done by:
...
```bash
sudo apt install ./rolvdesktop_2.1.3_amd64.deb
```
...
a file named __rolvdesktop-2.1.3-1.x86_64.rpm__.Now, let’s walk through the steps to install RolvDesktop:

1. Open the terminal ( press Ctrl + Alt + T or search for ‘Terminal’ in your applications menu)

2. Move to the directory in which you downloaded __rolvdesktop-2.1.3-1.x86_64.rpm__. This can be done by:
...
3. Run the following command and fill in your system password:
```bash
sudo rpm -ivh ./rolvdesktop-2.1.3-1.x86_64.rpm
```
...
![chat interface](chat interface.jpg)
...
# Shortcuts
## Focus Paste
The *FocusPaste* shortcut focuses the app and pastes clipboard content into the chat input field.

- **Features**:
- Combines window focus and paste in one action
- Customizable through Menu Bar → Shortcuts → Focus and Paste

![focus paste](focus_paste.png)



## Companion Window
The *Companion Window* feature allows users to open a mini chat window while keeping their main workspace active.

-**Features**:
- Opens a detached mini chat that interacts with the main Rolv Desktop window
- Includes a context panel for editing and reviewing messages
- Starts a new chat session each time it’s opened
- Accessible from Menu Bar → Shortcuts → Companion Window (Ctrl + Shift + P)

![companion window](companion-window.png)
...
the file’s location, simply open the Chat settings menu from the left sidebar. A a popup menu opens.

![Side bar](side bar.jpg)

From the popup menu, select the directory where your files are located. Any subdirectories within that folder will also be included in the search.
![Chat settings pop-up](select directory.jpg)



Once we have done this, we have a few other options that we can use.
...
Once you have built your workflow in the graph interface, you can now export the entire graph as an R Markdown (.Rmd) file or as a PDF for easy sharing and reproducibility.

You will see two new buttons in the graph interface toolbar:
![Export buttons](export_buttons.png)

- Export as RMD – Saves the full workflow as an R Markdown file, including all the code from each node in the correct execution order along with any plots or figures generated by those steps. This file can be opened in RStudio or any text editor to reproduce the entire analysis step-by-step.

- Export as PDF – Saves a PDF version of the workflow, which is ideal for sharing with collaborators or including in reports. The PDF contains the combined code, textual outputs, and all generated plots, making it a ready-to-read summary of your analysis.

When exporting as an RMD file, you can continue editing or running the analysis outside of ROLV. The PDF export, on the other hand, is static. It is perfect for documentation or presentations.
...
![Location of ask a human button](Ask human.jpg)
...
![Ask a human pop-up](Ask a human.jpg)

## Rolv's Intelligent Agent

The Intelligent Agent enables autonomous task handling across ROLV Desktop, allowing dynamic
decision-making and automated execution of queries (e.g., PubMed search, GEO dataset retrieval,
or code generation).

Locate the Agent icon on the left and click it to activate the system.
Once enabled, the agent will automatically detect the context of your query, route it to
 the appropriate sub-agent, and stream live responses such as “Analyzing query…” or “
 Executing PubMed agent…” directly within the chat.

When disabled, queries will be processed in manual mode — without orchestration or sub-agent routing.

![Intelligent Agent](Intelligent Agent.png)

![Agent query](Agent query.png)
